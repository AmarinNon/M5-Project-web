  ...
  <div class="row">
    <div class="col-sm-4">
      <select id="filter_subject" class="form-control" onchange="location = this.value;">
        ...
        <option value="attendance.php?action=history&id=0" <?php echo ($filter == 0)? 'selected':'' ;?>>Homeroom ชั้น <?php echo $homeroom->name;?></option>
    ...
    <tbody>
      ...
      <tr>
        ...
        for($j = 0 ; $j < count($checkin) ; $j++) {
          ...
          echo '<td>';
          if($checked) {
            echo '<span class="text-success">เข้าเรียน</span>';
          } else {
            echo '<span class="text-danger">ขาดเรียน</span>';
          }
          echo '</td>';
        }
        ?>
