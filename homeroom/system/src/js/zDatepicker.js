...
function zDatepicker(id) {
	...
	var __construct = function() {
		...
		if(jQuery("#" + elementID).data("yearoffset") != null && jQuery("#" + elementID).data("yearoffset").length != 0) {
			yearOffset = parseInt(jQuery("#" + elementID).data("yearoffset"), 10);
		} else {
			yearOffset = 100;
		}
		...
		if(jQuery("#" + elementID).data("setnull") != null && jQuery("#" + elementID).data("setnull") == true) {
			setNull();
		}
		return this;
	}();
	...
	function appendHTML() {
		...
		$el.html('');
		$el.append('<select class="form-control" name="' + elementID + '_day" id="' + elementID + '_day" ' + disabled + '></select>');
		$el.append('<select class="form-control" name="' + elementID + '_month" id="' + elementID + '_month" ' + disabled + '></select>');
		$el.append('<select class="form-control" name="' + elementID + '_year" id="' + elementID + '_year" ' + disabled + '></select>');

		$el.append('<input name="' + elementID + '" type="hidden" value="' + date + '">');

		$el.html('<span class="form-inline">' + $el.html() + '</span>');
	...
	function addYear() {
		...
		if(startDate != null) {
			...
			s = new Date().getFullYear() - yearOffset;
		}
		if(endDate != null) {
			var tmp = endDate.split("-");
			e = parseInt(tmp[0], 10);
		} else {
			e = new Date().getFullYear() + yearOffset;
		}
		...
		if(isContain) {
			jQuery("#" + elementID + "_year").val(year);
		} else {
	...
	function addMonth() {
		...
		var isContain = false;
		html += '<option value="">เดือน</option>';
		for(var i = s ; i <= e ; i++) {
			if(i == month) {
				isContain = true;
			}
	...
	function setNull() {
		jQuery("#" + elementID + "_year").val('');
		jQuery("#" + elementID + "_month").val('');
		jQuery("#" + elementID + "_day").val('');
		jQuery("input[name=" + elementID + "]").val('')
	}
...
function zRangeDatepicker(id) {
	...
	function appendHTML() {
		...
		jQuery("#" + elementID).append('<span class="zDatepicker" id="' + elementID + '_fromDate"></span> -- ');
